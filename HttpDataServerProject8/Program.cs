...
namespace HttpDataServerProject8
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // netsh http add urlacl url=http://+:11008/ user=DOMAIN\user
            if (args != null)
            {
            ...
            Log.Write(String.Format("Start accept on 'http://{0}:11008/' with MainSqlServerDataSource: '{1}'.", HostIPv4, MainSqlServerDataSource));
            server = new HttpServer();
            server.OnIncomingRequest += new HttpServer.RequestDelegate(OnIncomingRequest);
            server.Start("http://+:11008/");
        }
                ...
                if (Http.RequestIsAcceptable(request))
                {
                    ...
                    if (!String.IsNullOrWhiteSpace(body))
                    {
                        if (body[0] == '<')
                        {
                            ...
                            {
                                // исполнение запроса
                                ResponsePackage rsp = CommandSelector.Execute(rqp);
                                // запись ответа
